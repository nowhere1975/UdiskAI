import { app, BrowserWindow, ipcMain, session, nativeTheme, dialog, shell, nativeImage, Menu, protocol, net } from 'electron';
 ...
 * Returns the bridge config (callbackUrl, secret, tools).
 */
 ...
 * stop existing MCP servers → restart with new config.
...
const refreshMcpBridge = (): Promise<{ tools: number; error?: string }> => {
  ...
  mcpBridgeRefreshPromise = (async () => {
    try {
      ...
      console.log(`[McpBridge] refresh complete: ${toolCount} tools`);
  ...
  ipcMain.handle('cowork:config:set', async (_event, config: {
    ...
    try {
      ...
      if (normalizedConfig.workingDirectory !== undefined && normalizedConfig.workingDirectory !== previousWorkingDir) {
        ...
        if (syncResult.error) {
          console.warn('[Cowork Memory] Workspace sync failed:', syncResult.error);
        }
        // Ensure IDENTITY.md has default content in the new workspace
        try {
          ensureDefaultIdentity(normalizedConfig.workingDirectory);
        } catch (err) {
          console.warn('[Cowork] ensureDefaultIdentity failed (non-fatal):', err);
        }
      }
      return { success: true };
    } catch (error) {
