...
- **解压即用** — 下载 zip，解压后双击 `启动.bat` 运行，无需安装程序，不写注册表
...
3. **双击 `启动.bat`** 启动程序（勿直接双击 `LobsterAI.exe`）
4. 在设置页面填入 API Key，即可开始使用

> **为什么要用 `启动.bat`？**
> `启动.bat` 会设置 `PORTABLE_MODE=1` 环境变量，告知程序将所有数据存储到本地 `data/` 目录，而非系统的 AppData 文件夹。直接运行 `LobsterAI.exe` 则会按普通安装版模式运行，数据不在本地目录，失去"便携"特性。
